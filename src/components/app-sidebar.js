...
import { Calendar, HousePlus, Inbox, Search, Settings } from "lucide-react"
...
export function AppSidebar({collapsible, className = ''}) {
    const pathname = usePathname()
    return (
        <Sidebar className={`bg-primary border-none ${className}`} collapsible={collapsible} >
            <SidebarHeader className="h-16 bg-primary flex justify-start px-4 align-middle">
                {/* <Link href='/' title="Youtube Home">
                    <svg xmlns="http://www.w3.org/2000/svg" id="yt-ringo2-svg_yt9" width="100" height="40" viewBox="0 0 93 20" focusable="false" aria-hidden="true">
                        <g>
                            <path d="M14.4848 20C14.4848 20 23.5695 20 25.8229 19.4C27.0917 19.06 28.0459 18.08 28.3808 16.87C29 14.65 29 9.98 29 9.98C29 9.98 29 5.34 28.3808 3.14C28.0459 1.9 27.0917 0.94 25.8229 0.61C23.5695 0 14.4848 0 14.4848 0C14.4848 0 5.42037 0 3.17711 0.61C1.9286 0.94 0.954148 1.9 0.59888 3.14C0 5.34 0 9.98 0 9.98C0 9.98 0 14.65 0.59888 16.87C0.954148 18.08 1.9286 19.06 3.17711 19.4C5.42037 20 14.4848 20 14.4848 20Z" fill="#FF0033"></path>
                            <path d="M19 10L11.5 5.75V14.25L19 10Z" fill="white"></path>
                        </g>
                        <g id="youtube-paths_yt9" fill="#fff">
            ...
            <SidebarContent className="bg-primary text-foreground">
                <ScrollArea className="pr-3">
                                ...
                                {mainMenu.map((item) => (
                                    <SidebarMenuItem key={item.title}>
                                        <SidebarMenuButton asChild>
                                            <Link className={`group-data-[collapsible=icon]:!size-12 text-lg font-semibold [&>svg]:size-6 py-5 px-3 flex gap-6 rounded-xl group-data-[collapsible=icon]:!p-3  ${pathname === item.url ? 'bg-sidebar-accent' : ''}`} href={item.url}>
                                                <item.icon size={80} />
                    ...
                    <SidebarSeparator className="opacity-15 h-[2px] my-3" />
                    <SidebarGroup>
                        <SidebarGroupLabel className="text-foreground pb-5 px-3 text-md font-semibold">Application</SidebarGroupLabel>
                        <SidebarGroupContent>
                            <SidebarMenu>
                                {items.map((item) => (
                                    <SidebarMenuItem key={item.title}>
                                        <SidebarMenuButton asChild>
                                            <Link href={item.url} className={`group-data-[collapsible=icon]:!size-12 text-lg font-semibold [&>svg]:size-6 py-5 px-3 flex gap-6 rounded-xl group-data-[collapsible=icon]:!p-3 ${pathname === item.url ? 'bg-sidebar-accent' : ''}`}>
                                                <item.icon size={100} />
                                                <span>{item.title}</span>
                                            </Link>
                                        </SidebarMenuButton>
